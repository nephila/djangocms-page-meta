...
class TemplateMetaTest(BaseTest):
    def test_page_meta(self):
        ...
        test1 = page.changed_date.isoformat()
        self.assertContains(response, '<meta name="twitter:domain" content="example.com">')
        # self.assertContains(
        #     response, '<meta itemprop="datePublished" content="%s">' % page.changed_date.isoformat()
        # )
        # self.assertContains(
        #     response, '<meta property="article:expiration_time" content="%s">' % page.publication_end_date.isoformat()
        # )
    ...
    def test_title_meta(self):
        ...
        """
        # page1, __ = self.get_pages()
        page_en = create_page(title='test', template="page_meta.html", language='en')
        page_it = create_page(title='test', template="page_meta.html", language='it')
        ...
        # Italian language
        # response = self.client.get(page_it.get_public_url("it"))
        # response.render()
        ...
        # self.assertContains(
        #     response, '<meta property="og:url" content="http://example.com%s">' % page_it.get_public_url("it")
        # )
        # self.assertContains(response, '<meta custom="attr" content="foo-it">')

        # English language
        # response = self.client.get(page_en.get_public_url("en"))
        ...
        self.assertContains(response, '<meta property="og:title" content="test">')
        self.assertContains(
    ...
    #
        title2_en = page_en.get_title_obj(language="en", fallback=False)
